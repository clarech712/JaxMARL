...
def plot_contributions(idx, state_seq, mech, rival_mechs, gen, config):
    ...
    for idx in range(5):
        ...
        # Save plot
        codename = f"RS_ps{config['population_size']}_ss{config['selected_size']}_es{config['elite_size']}_ng{config['num_generations']}_mr{config['mutation_rate']}_e{config['eta']}"
...
def genetic_algorithm(tails, config, key):
    ...
    for gen in range(num_generations):
        ...
        key, ind_key = jax.random.split(key, 2)
        indices = jnp.arange(population_size)
        selected_indices = jax.random.choice(ind_key, indices, shape=(selected_size,), replace=False)
        ...
        key, ind_key = jax.random.split(key, 2)
        indices = jnp.arange(selected_size)
        elite_indices = jax.random.choice(ind_key, indices, shape=(elite_size,), replace=False)
